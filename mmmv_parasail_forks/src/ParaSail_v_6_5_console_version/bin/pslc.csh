...
set valid_flags = "o:O:cm:gikb:lvh"
...
set compiler_srcs = "$psl_dir/lib/reflection.ps? $psl_dir/lib/vn_il.psi $psl_dir/lib/vn_il_stub.psl $psl_dir/lib/llvm_printer.ps? $psl_dir/lib/compiler.ps?"
...
if ("$to_be_compiled" != "") then
   if ($use_interpreter == 1) then
      $psl_dir/build/bin/parasail_main $compiler_and_deps $files -command Compile $psl_flags $to_be_compiled
   else
      $psl_dir/bin/compiler.exe $comp_lib $to_be_compiled $psl_dir/lib/compiled_main.psl -command Compiled_Main $psl_flags $to_be_compiled
   endif
endif
...
if ($link == 1) then
   ...
   cd $psl_dir; make --silent $make_target OBJS="$objs" EXE="$exe" SILENT=1
endif

...
package body PSC.Interpreter is
   ...
   package Thread_Server_Info is
      ...
      Server_Stack_Size : constant := 300_000;
   ...
   procedure Select_Ancestor_Part
     (Context            : Exec_Context;
      Destination        : Word_Ptr;
      Source_Obj         : Word_Type;
      Ancestor_Type_Desc : Type_Descriptor_Ptr;
      Source_Type_Desc   : Type_Descriptor_Ptr;
      Is_Passed_By_Ref   : Boolean);
   --  We want to select ancestor part of type Ancestor_Type_Desc of an object
   --  (value in Source_Obj, of type Source_Type_Desc), and place ancestor
   --  object in Destination.
   --  If Is_Passed_By_Ref is True, then Source_Obj contains "ref" to object,
   --  and ref to ancestor part should be placed in Destination.
   ...
   begin
      ...
      Store_Word (Op_Desc_Phys, Large_Obj_Header_Size, Target_Addr);
   ...
   begin
      return Create_Operation_Desc (Context,
         (Operation_Locator_Base, Operation_Locator_Offset),
   ...
   begin
      ...
      Context.Local_Stg_Rgn := Old_Rgn.Enclosing_Stg_Rgn;
   ...
   begin
      if Finish_Type_Descriptor_Ptr /= null then
         ...
         Messages.Put_Error ("Need compiler to finish type descriptor " &
   ...
   begin
      ...
      if Source_Type_Desc.Is_Polymorphic then
         ...
         begin
            ...
            if Is_Passed_By_Ref then
               --  Store back a "ref"
               Store_Word_Ptr (Destination, 0, Ances_Obj_Addr);
            else
               --  Store back a "value"
               Store_Word (Destination, 0, Ances_Obj);
            end if;
   ...
   is
      ...
      Source_Obj : Word_Type := Poly_Obj;
   begin
      --  Pass the buck to Select_Ancestor_Part which checks
      --  for Is_Polymorphic case.
      if Polymorphic_Ancestor_Lvalue then
         --  Select_Ancestor_Part expects the address, not the value
         Source_Obj := Word_Ptr_To_Word
           (Locator_To_Physical_Address (Context, Source));
      end if;

      Select_Ancestor_Part
        (Context,
         Destination => Locator_To_Physical_Address (Context, Destination),
   ...
   begin
      if Existing_Obj_In_Stg_Rgn = Null_Object_Locator then
   ...
   begin
      if Is_Small (Type_Desc) then
         ...
         Store_Word
           (Context,
            Destination,
            Null_For_Stg_Rgn (Local_Stg_Rgn (Context)));
      end if;
   ...
   protected body Thread_Manager is
      ...
      entry Get_Thread
        (Server_Index : Thread_Server_Index;
         Tcb_To_Run   : out Word_Ptr)
         when Num_Waiting_Shared_Threads > 0
           or else Is_Shut_Down
           ...
           when Is_Shut_Down
             ...
             or else --  Handle case when we exhaust existing servers
               ...
               (Get_Thread'Count = 0
                ...
                and then Num_Dynamically_Allocated_Thread_Servers >=
                  Max_Dynamically_Allocated_Thread_Servers)
      is
      begin
      ...
      begin
         ...
         for I in Server_Info_Array'Range loop
            declare
         ...
         begin  --  Dump_One_Master
            ...
            while Subthread /= null loop
               ...
               begin
                  ...
                  while Submaster /= Encloser and then Submaster /= 0 loop
   ...
   protected body Stg_Rgn_Manager_Type is
      ...
      begin
         ...
         Chunk.Next_Chunk := Enclosing_Stg_Rgn.First_Chunk;
         Enclosing_Stg_Rgn.First_Chunk := Chunk;
         if Chunk.Next_Chunk = null then
            --  Is now the one and only chunk of enclosing region
            Enclosing_Stg_Rgn.Last_Chunk := Chunk;
         else
            --  Set up back link to this chunk
      ...
      protected body Delay_Queue is
         ...
         begin
            ...
            if Delay_Queue_Head /= null
              and then Delay_Queue_Head.Delay_Until <= Now
            then
   ...
   package body Large_Obj_Header_Ops is
      ...
      begin
         if Large_Obj_Addr < 0 and then Large_Obj_Addr mod 2 = 1 then
            ...
            return Stg_Rgn_Index ((-Large_Obj_Addr) rem Chunk_Divisor / 2);
         else
   ...
   package body Locked_And_Queued is
            ...
            declare
               ...
               Params : constant Word_Ptr :=
                 Locator_To_Physical_Address
                    (Context,
                     (Local_Area, Local_Area_Local_Data_Offset));
            ...
            begin
               ...
               Execute_Nested_Block
                 (Context,
                  ...
                  Static_Link =>
                    Locator_To_Physical_Address (Context, (Local_Area, 0)),
                  Code_Block =>
      ...
      is
         ...
         New_Context : Exec_Context :=
           (Enclosing_Type    => Context.Enclosing_Type,
            Local_Stg_Rgn     => Get_New_Local_Stg_Rgn
               ...
               begin
                  ...
                  if Debug_Calls or Debug_Threading then
                     ...
                     Put_Line
                       (" (param area, 0) =" &
                        Hex_Image
                           (Fetch_Word (New_Context, (Param_Area, 0))));
                  end if;
               end;
            else
   ...
   package body PSVM_Thread_Support is
      ...
      begin
         ...
         if Subthread_Of_Master > 0 then
            --  Check whether Tcb uses queueing, and if say
            --  keep looking for TCB that is subthread of given master.
            while Tcb_To_Run /= null
   ...
   package body Type_Descriptor_Ops is
      ...
      is
         pragma Assert (Is_Small (Type_Desc));
      begin
         return Skip_Over_Op_Map (Type_Desc).Null_Value;
      end Null_For_Type;
      ...
      begin
         if Type_Desc.Has_Op_Map or else Type_Desc.Is_Polymorphic then
            ...
            begin
               ...
               case Param.Kind is
                  ...
                  when Formal_Operation =>
                     Messages.Put_Error ("NYI: Formal Operations of Modules",
                       Src_Pos => Source_Positions.Null_Source_Position);
               end case;
            end;
         end loop;
   ...
   package body Debug is
      ...
      begin
         if Code = null then
            ...
            for I in 1 .. Code.Code_Length loop
               Dump_One_Instruction (Code.Instrs (I));
      ...
      begin
         ...
         if Is_Large_Null (Value) then
            ...
            Put_Line
              (Indent_Str &
               "special value " &
               Hex_Image ((-Value) / Chunk_Divisor) & " (large -- region" &
               Stg_Rgn_Index'Image (Large_Obj_Stg_Rgn_Index (Value)) &
               ")");
         else
            --  Dump each component
            ...
            then
               ...
               begin
                  if Target_Routine /= null then
                     Put (" = " & Strings.To_String (Target_Routine.Name));
                  end if;
               end;
            end if;
      ...
      begin  --  Dump_One_Instruction
         ...
         case Op is
            ...
            when Call_Op | Call_Nested_Block_Op | Check_Nested_Block_Op =>
               ...
               case Op is
                  when Call_Op =>
                     ...
                     Put (", Target_Index =>" &
                          Routine_Index'Image (Instr.Target_Index));
            ...
            when Store_Int_Lit_Op                    |
               ...
               case Op is
                  ...
                  when Store_Str_Lit_Op =>
                     ...
                     if Instr.Existing_Str_In_Stg_Rgn /=
                       Null_Object_Locator
                     then
            ...
            when Start_Parallel_Op      |
               ...
               case Op is
                  when Start_Parallel_Op      |
                     ...
                     Put
                       (", Num_Params =>" &
                        Natural'Image (Instr.Num_Params));
         ...
         if not Code.Is_PSVM_Routine then
            ...
            if Code.Code /= null then
               ...
               for I in 1 .. Code.Code.Code_Length loop
                  Dump_One_Instruction (Code.Code.Instrs (I));
               end loop;
            end if;
         end if;
      end Dump_Routine;
   ...
   --  Space is uninitialized.
   --  Caller gets a lock if needed.
      Chunk      : Stg_Rgn_Chunk_Ptr := Stg_Rgn.First_Chunk;
      Last_Chunk : constant Stg_Rgn_Chunk_Ptr := Stg_Rgn.Last_Chunk;
   begin
      while Chunk /= null loop
         if Chunk.Space_Left >= Size_In_Words then
            ...
            if Chunk.Prev_Chunk /= null then
               --  Make prev chunk the last chunk
               Chunk.Prev_Chunk.Next_Chunk := null;
               Stg_Rgn.Last_Chunk.Next_Chunk := Stg_Rgn.First_Chunk;
               Stg_Rgn.First_Chunk.Prev_Chunk := Stg_Rgn.Last_Chunk;
               Stg_Rgn.Last_Chunk := Chunk.Prev_Chunk;
               Chunk.Prev_Chunk := null;
               --  Make this chunk the first chunk
      ...
      if Chunk = null then
         ...
         --  Hook chunk into this region
         Chunk.Prev_Chunk := null;
         Chunk.Next_Chunk := Stg_Rgn.First_Chunk;
         if Stg_Rgn.First_Chunk /= null then
            --  Set back pointer
            Stg_Rgn.First_Chunk.Prev_Chunk := Chunk;
         else
            --  Only chunk in region
            Stg_Rgn.Last_Chunk := Chunk;
         end if;
         Stg_Rgn.First_Chunk := Chunk;
   ...
   begin
      ...
      Result := Enclosing_Stg_Rgn.First_Chunk;
      while Result /= null loop
      ...
      if Result /= null then
         ...
         if Result.Prev_Chunk /= null then
            --  Was not the first chunk
         ...
         if Result.Next_Chunk /= null then
            --  Was not the last chunk
            Result.Next_Chunk.Prev_Chunk := Result.Prev_Chunk;
         else
            --  Was the last chunk
            Enclosing_Stg_Rgn.Last_Chunk := Result.Prev_Chunk;
         end if;
   ...
   begin
      ...
      declare
         ...
         New_Context : Exec_Context :=
           (Enclosing_Type      => Get_Enclosing_Type (Static_Link),
            ...
            Control_Area        => Cur_State_Context.Control_Area,
            Open_Master         => null,
            Server_Index        => Server_Index);
      ...
      begin --  Call_Through_Operation_Desc
         ...
         if not Target_Routine.Is_PSVM_Routine then
            --  Reuse prior stg rgn
            New_Context.Local_Stg_Rgn := Cur_State_Context.Local_Stg_Rgn;
         else
   ...
   begin
      ...
      --  Store length
      Store_Word (New_Obj + Large_Obj_Header_Size, Word_Type (Array_Len));
      ...
      if Init_Elements then
         if Is_Small (Element_Type) then
            ...
            for I in 1 .. Offset_Within_Area (Array_Len) loop
               Store_Word (New_Obj + Large_Obj_Header_Size + I, Element_Value);
            end loop;
         else
            ...
            for I in 1 .. Offset_Within_Area (Array_Len) loop
               ...
               begin
                  Store_Word
                    (New_Obj + Large_Obj_Header_Size + I, Copy_Of_Val);
         ...
         for I in 1 .. Offset_Within_Area (Array_Len) loop
            ...
            Store_Word (New_Obj + Large_Obj_Header_Size + I,
               Null_For_Type_Or_Stg_Rgn
   ...
   is
      ...
      begin
         if Doing_Run_Time_Checks then
            ...
            then
               ...
               --      For now we will dump the stack and keep going
               Set_Output (Current_Error);
               Dump_Stack (Cur_State, Num_Stack_Frames => 1);
               Set_Output (Standard_Output);
            end if;
         end if;
      ...
      Old_State : constant Server_State := Cur_State;
...
--       end if;
      ...
      if not Instructions.Is_PSVM_Routine then
         ...
         then
            ...
            --  No result returned
            Instructions.Routine_Addr (Context,
              Context.Params,
      ...
      loop
         ...
         begin
            ...
            when Store_Str_Lit_Op =>
               ...
               begin
                  if Instr.Existing_Str_In_Stg_Rgn /= Null_Object_Locator then
            ...
            when Create_Polymorphic_Obj_Op =>
               ...
               Create_Polymorphic_Obj (Context,
                  Stg_Rgn_Of_Existing_Large_Obj (Context, Instr.Source),
   ...
   exception
      when E : others =>
         Messages.Put_Error
           ("Exception " & Ada.Exceptions.Exception_Name (E) & " raised.",
            Src_Pos => Execution_Source_Pos (Server_Index));
   ...
   begin --  Execute_Call_Op
      ...
      if Locked_Param_Info.Param_Index > 0 then
         ...
         Target_Routine.Routine_Addr (New_Context,
           New_Context.Params,
           To_Non_Op_Map_Type_Desc (Static_Link));
            --  NOTE: This is an unchecked conversion and we are
            --        relying on no discriminant check being performed
      ...
      if Polymorphic_Output_Type /= null then
         ...
         --  TBD: Handle multiple outputs
         declare
            Output_To_Be_Wrapped : constant Word_Ptr :=
         ...
         begin
            ...
            Set_Large_Obj_Header
              (Poly_Obj,
               ...
               Polymorphic_Output_Type.Index);

            --  Initialize only "component" to underlying value
            Store_Word (Poly_Obj + Large_Obj_Header_Size,
              Content_Of_Physical_Address (Output_To_Be_Wrapped));

            --  Replace Output
            Store_Word (Output_To_Be_Wrapped, 0, Poly_Obj);
   ...
   begin
      ...
      if Code_Address = null or Op_Desc_Virt_Addr /= Null_Virtual_Address then
         Find_Routine_Context
           (Context,
            Routine_Locator => (Target_Base, Target_Offset),
            Static_Link => (Static_Link_Base, Static_Link_Offset),
            Params => Params,
      ...
      if Type_Desc = null then
         ...
         declare
            ...
            Real_Type_Desc : constant Non_Op_Map_Type_Ptr :=
              Skip_Over_Op_Map (Type_Desc);
         begin
            if Real_Type_Desc.Is_Small then
               --  Each small type might have its own rep for null
               return Word = Real_Type_Desc.Null_Value;
               ...
               return Real_Type_Desc.Is_Polymorphic
                 and then Is_Null_Value  --  Recurse with underlying value/type
   ...
   begin
      return Value mod 2 = 1
        and then Value / Chunk_Divisor = Large_Null_Chunk;
   end Is_Large_Null;
   ...
   begin
      return Skip_Over_Op_Map (Type_Desc).Is_Small;
   end Is_Small;
   ...
   begin
      return Value < 0 and then Value mod 2 = 1
        and then Value / Chunk_Divisor in
          Large_Null_Chunk * 4 .. Large_Null_Chunk;
   ...
   is
      use type Trees.Root_Sem_Ptr;
   begin
      if Now_Building /= null and then Location = (Type_Area, 0) then
         --  Self reference
         return Now_Building;
      end if;

      if Location = Null_Object_Locator
      ...
      pragma Assert (Location /= Null_Object_Locator);
   ...
   begin
      if Stg_Rgn.Null_Value = Null_Virtual_Address then
         --  Need to compute a "null" for this region
         --  NOTE: We reserve a special negative chunk # for large nulls
         Stg_Rgn.Null_Value := Large_Null_Chunk * Chunk_Divisor -
                              Word_Type (Stg_Rgn.Index) * 2 - 1;
      end if;
   ...
   --  associated with the existing object.
      Type_Desc : constant Non_Op_Map_Type_Ptr := Skip_Over_Op_Map (Type_Info);
   begin
      if Type_Desc.Is_Small then
         --  Return appropriate kind of "null"
   ...
   procedure Register_Compiled_Operations
     (Num_Operations  : Local_Operation_Count;
      ...
      Operation_Uses_Queuing : Operation_Uses_Queuing_Table;
   ...
   begin
      for I in 1 .. Num_Operations loop
         ...
         begin
            ...
            --  Fill in Address of operation and Uses_Queuing flag
            Compiled_Routine.Routine_Addr := Operation_Addrs (I);
            Compiled_Routine.Uses_Queuing := Operation_Uses_Queuing (I);
   ...
   --  Put region on free list, and null out Stg_Rgn parameter.
      ...
      Chunk_Ptr    : Stg_Rgn_Chunk_Ptr := Stg_Rgn.First_Chunk;
   ...
   begin
      ...
      Stg_Rgn.Last_Chunk := null;
      Stg_Rgn.Null_Value := Null_Virtual_Address;
   ...
   function Locator_To_Physical_Address_Exported
     (Context : Exec_Context;
      ...
      Locator : constant Object_Locator := (Base, Offset);
   begin
   ...
   package body Thread_Server_Info is
      ...
      procedure Dump_Stack
        (State            : Server_State;
         Num_Stack_Frames : Positive := Positive'Last) is
      begin
         if State.Code /= null then
            ...
            if not State.Code.Is_PSVM_Routine then
               ...
               Dump_One_Instruction (State.Code.Code.Instrs (State.Pc));
            else
               Put_Line

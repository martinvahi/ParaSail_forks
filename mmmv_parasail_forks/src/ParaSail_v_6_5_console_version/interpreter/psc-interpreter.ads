...
package PSC.Interpreter is
   ...
   Large_Null_Chunk : constant Object_Virtual_Address := -2 ** 25;
   ...
   Null_Object_Locator : constant Object_Locator := (Zero_Base, 0);

   Unknown_Func_Type_Obj_Locator : constant Object_Locator :=
     (Zero_Base, Offset_Within_Area'Last);  --  Special locator of func-type
   ...
   Thread_Control_Block_Size : constant Offset_Within_Area := 10;
   ...
   --  single-threaded within a chunk.
      ...
      Null_Value : Word_Type := Null_Virtual_Address;    --  Null for
                                                         --  this region
   ...
   --  Table of uses-queuing flags of compiled operations
   type Operation_Uses_Queuing_Table is
     array (Local_Operation_Index) of Boolean;
   for Operation_Uses_Queuing_Table'Component_Size use 8;

   --  Use very large constant bounds so that we can pass
   --  llvm pointers and Ada won't search for the bounds at run time

...
package body PSC.Trees.Semantics.Dynamic is
   ...
   Checking_For_Nulls : Boolean := True;
         ...
         if Debug_Pre_Cg then
            Put_Line
              (" For operation " &
               ...
               ", Num_Nested_Blocks = " &
               Natural'Image (Num_Nested_Blocks (T.Statements)) &
               ", Uses_Queuing = " &
   ...
   begin
      ...
      begin
         ...
         if Is_Slow_Call and then Comp_Sem.Slow_Calls /= Slow_Call then
   ...
   begin
      ...
      --  Combine the slow-call, queuing, and nested-block info
      ...
      case T.Operator is
         when Stmt_Ops | Short_Circuit_Ops =>
      ...
      if Eval_In_Parallel then
         ...
         begin
            ...
            if T.Operator = Parallel_Stmt_Op then
              ...
              and then Comp_Sem.Slow_Calls /= Slow_Call
            then
   ...
   begin
      ...
      if Annotation_Sem /= null then
         Annotation_Sem.Num_Nested_Blocks :=
           Combine_Operand_Blocks (T.Annotations) + 1; --  for the annotations
   ...
   begin
      ...
      if For_Loop_Sem.Uses_Parallel_Nested_Block then
         ...
         For_Loop_Sem.Slow_Calls :=
           Combine_Sequential_Computations (Slow_Calls (T.Loop_Body),
             Slow_Calls (T.End_With_Values));
      ...
      For_Loop_Sem.Uses_Queuing := Combine_Operand_Queuing (T.Iterators)
                                  ...
                                  or else    --  NOTE: Filter should not use
                                             --  queuing
                                   Uses_Queuing (T.Loop_Body)
                                  or else Uses_Queuing (T.End_With_Values);
      if Debug_Pre_Cg then
         Put_Line
   ...
   procedure Emit
     (Visitor : in out Code_Gen_Visitor;
      Instr : Interpreter.Instruction) is
      ...
      case Instr.Op is
         ...
         when Store_Address_Op =>
            ...
            if Instr.Source = Instr.Destination then
               Sem_Error
                 ("Internal: Store_Address_Op with Source = Destination",
   ...
   procedure Check_For_Null_Return
     (Visitor : in out Code_Gen_Visitor;
      ...
      Outputs : Lists.List renames
        Operation.Tree (Tree_Ptr_Of
                           (Op_Sem.Definition).all).
                              Operation_Outputs;
   begin
      if Checking_For_Nulls and then Lists.Length (Outputs) > 0 then
         declare
            use Interpreter;
         ...
         begin
            ...
            if Output_Sem.Resolved_Type.Value_Is_Optional
              or else
                Param_Decl.Tree (Tree_Ptr_Of (First_Output).all).Is_Optional
      ...
      Mini_Op : constant Optional_Tree :=
        Operation.Make
           (Name => Null_Optional_Tree,   --  TBD: Qualified_Name
            ...
            Preconditions => Lists.Empty_List,
            Postconditions => Lists.Empty_List,
            Is_Def => True,
            Statements =>
              Control_Stmt.Make
      ...
      if Computation /= null then
         declare
            ...
            Type_Desc : constant Type_Descriptor_Ptr :=
              Known_Type_Desc
                (Run_Time_Type_Info (Operand_Sem.Resolved_Type));
         ...
         begin
            ...
            Result := Fetch_Word (Context, (Param_Area, 0));
   ...
   begin
      ...
      if Operand_Sem.Target_Polymorphic_Type /= null then
         Type_Desc :=
            Known_Type_Desc
              (Run_Time_Type_Info (Operand_Sem.Target_Polymorphic_Type));
      else
         Type_Desc :=
            Known_Type_Desc (Run_Time_Type_Info (Operand_Sem.Resolved_Type));
      end if;
   ...
   procedure Emit_Code_For_Resolved_Tree
     (T : Optional_Tree;
      ...
      Tree_Sem : constant Sem_Ptr := Sem_Ptr (Underlying_Sem_Info (T_Copy));
   begin
      if not Visitor.Is_Lvalue_Context
        and then not Visitor.Gen_Parallel_Invocations_Only
        and then Tree_Sem /= null
        and then Tree_Sem.all in Operand_Semantic_Info'Class
      ...
      then
         ...
         begin
            if Static.Known_To_Be_Small (Opnd_Sem.Resolved_Type)
              and then Orig_Target_Object = (Local_Area, Orig_Target_Offset)
            then
   ...
   begin
      ...
      then
         ...
         if Info.Index /= 0 then
            ...
            if Type_To_Use.U_Base_Type /= Obj_Type.U_Base_Type then
   ...
   begin
      if Existing_Type_Desc /= null then
         --  Just copy over the pre-assigned locator and fill in the type-sem.
         Obj_Type.Type_Descriptor_Location := Existing_Type_Desc.Location;
            ...
            begin
               if Op_Map_Target_Ref = null then
                  ...
                  if Nested_Type.Is_Formal_Type then
                     ...
                     begin
                        if Progenitor /= null
                          and then Progenitor.U_Base_Type /=
                            Resolved_Nested_Type.U_Base_Type
                        then
   ...
   begin
      ...
      if Type_Desc.Num_Operations > 0 then
         declare
      ...
      if Type_Desc.Num_Components > 0 then
         ...
         for I in 1 .. Type_Desc.Num_Components loop
            declare
               ...
               Comp_Actual_Type : constant Type_Sem_Ptr :=
                 Static.Substitute_Actuals
                    (Comp_Formal_Type,
                     U_Base_Type_Region (Obj_Type));
      ...
      if Type_Desc.Num_Actuals_Of_Formals > 0 then
         --  Create "actual" tree for each parameter
         --  Create op-map as well
      ...
      if Type_Desc.Num_Nested_Consts > 0
      ...
      if Obj_Type.Associated_Module.Needs_Polymorphic_Type_Desc then
                                 ...
                                 then
                                    exit when Static.Signatures_And_Modes_Match
                                       (Operation_Sem_Ptr (Actual_Op.Sem_Info),
                                        Formal_Op_Sem,
                                        Extra_Subst1 =>
                                          Obj_Type.U_Base_Type,
                                        Extra_Subst2 =>
                                          Progenitor_Type.U_Base_Type,
                                        Substitute_Using1 =>
                                           U_Base_Type_Region
                                             (Obj_Type),
                                        Substitute_Using2 =>
   ...
   is
      ...
      --        This does not include types that are merely "implemented."
         ...
         Obj_Base_Type : constant Type_Sem_Ptr := Obj_Type.U_Base_Type;
         Ancestor : Type_Sem_Ptr := Obj_Base_Type;
      begin
         --  Count up ancestors
         ...
         begin
            Ancestor := Obj_Base_Type;
            --  Fill table in reverse order
            for I in reverse 1 .. Num_Ancestors loop
               Result (I) := Known_Type_Desc
                              (Run_Time_Type_Info (Ancestor.U_Base_Type));
               Ancestor := Ancestor.Parent_Type;
            end loop;
         ...
         begin
            ...
            then
               Poly_Type_Desc.Root_Type_Desc :=
                 Known_Type_Desc (Run_Time_Type_Info (Formal_Type));
   ...
   is
      ...
      begin
         ...
         then
            ...
            return (Type_Area, Type_Area_Offset);
         else
                  ...
                  then
                     ...
                     begin
                        ...
                        then
                           ...
                           --  Type_Area.
                           return (Type_Base, Type_Area_Offset);
                        end if;
                     end;
                  end if;
   ...
   begin  --  Run_Time_Type_Info
      ...
      if Polymorphic_Param_Index > 0 then
         ...
         return
           (Param_Area,
            Offset_Within_Area (Polymorphic_Param_Index) - 1);
      ...
      then
         ...
         declare
            function Non_Poly_Formal_Type return Type_Sem_Ptr is
            --  Return non-polymorphic version of Formal_Type
            begin
               if Formal_Type /= null
                 and then Formal_Type.Is_Polymorphic
               then
                  return Formal_Type.U_Base_Type;
               else
                  return Formal_Type;
               end if;
            end Non_Poly_Formal_Type;

            Base_Type_Info : constant Object_Locator :=
              Run_Time_Type_Info
                 (Obj_Type.U_Base_Type,
                  Referring_Module,
                  Non_Poly_Formal_Type,
                  Source_Pos);

            use Interpreter;
         begin
            if Base_Type_Info = Null_Object_Locator then
               --  Some prior error
               return Null_Object_Locator;
            elsif Base_Type_Info.Base /= Zero_Base then
               --  Convert base type into polymorphic type locator
               return (Base_Type_Info.Base, Base_Type_Info.Offset +
               ...
               return
                 Build_Polymorphic_Type_Desc
                    (Obj_Type.U_Type,
                     Formal_Type,
                     Source_Pos,
                     Known_Type_Desc (Base_Type_Info)).Location;
            end if;
         end;
      elsif Obj_Type.U_Base_Type /= Obj_Type then
         --  Get the run-time type info for the base type
      ...
      then
         ...
         --  Recompute if nested module might be involved
      ...
      if Debug_Code_Gen and then Result = Null_Object_Locator then
   ...
   procedure Init_Outputs_To_Null
     (Visitor : in out Code_Gen_Visitor;
      ...
      Enc_Module : constant Module_Sem_Ptr :=
        Static.Find_Enclosing_Module_Interface (Visitor.Decl_Region);
   begin
      Output_Inited_Null := False;  --  Default initialize OUT param.
      ...
      for I in 1 .. Num_Outputs loop
         ...
         begin
            ...
            then
               ...
               if Visitor.Target_Object /= Null_Object_Locator and then
                 not Static.Known_To_Be_Small (Resolved_Type)
               then
                  --  Use region of target
                  if Visitor.Target_Object /=
               ...
               when Type_Area | Enclosing_Type_Areas =>
                  ...
                  begin
                     ...
                     then
                        ...
                        begin
                           if Enc_Module = null
                             or else Prefix_Type.U_Base_Type /=
         ...
         then
            ...
            return
              (Type_Area,
               Type_Operation_Offsets'First +
                 Offset_Within_Area (Op_Sem.Equiv_To.Index));
            ...
            return
              (Enclosing_Type_Areas'First + 1,
               Type_Operation_Offsets'First +
                 Offset_Within_Area (Op_Sem.Index));
         else
            return
              (Type_Area,
               Type_Operation_Offsets'First +
   ...
   begin
      if Target_Object /= Null_Object_Locator
   ...
   begin
      if Visitor.Finalizable_Temp_Offset = 0 then
         if (Local_Area, Visitor.Target_Local_Offset - 1) = Current_Loc then
   ...
   procedure Emit_Nested_Block_Start
     (New_Visitor : in out Code_Gen_Visitor;
      ...
      New_Block_Index : constant Block_Index :=
        New_Visitor.Nested_Blocks.Num_Blocks_Generated + 1;
      Info : Block_Info renames New_Visitor.Nested_Blocks.Info (
        New_Block_Index);

   begin  --  Emit_Nested_Block_Start

      --  Allocate new block index
      New_Visitor.Nested_Blocks.Num_Blocks_Generated := New_Block_Index;

      --  Initialize nested block code pointer
      New_Visitor.Nested_Blocks.Info (New_Block_Index).Code := New_Code;
      ...
      Comp_Sem.Parallel_Result_Offset := New_Visitor.Target_Local_Offset;
      ...
      for I in Block_Inputs'Range loop
         ...
         Emit
           (New_Visitor,
            (Copy_Word_Op,
             Source_Pos => Find_Source_Pos (Comp_Sem.Definition),
             Destination => (Local_Area, New_Visitor.Target_Local_Offset),
             Dest_Name => Strings.Null_U_String_Index,
      ...
      case Instr_Opcode is
         when Start_Parallel_Op =>
            ...
            pragma Assert (New_Visitor.Local_Master > 0);
            if not New_Visitor.Master_Is_Started then
               --  First parallel invocation for this master

               --  Initialize First_Awaited_Block
               pragma Assert (New_Visitor.First_Awaited_Block = 0);
               New_Visitor.First_Awaited_Block := New_Block_Index;
               ...
               Emit
                 (New_Visitor,
                  (Start_Parallel_Op,
                   ...
                   Parallel_Static_Link => (Local_Area, 0),
                     --  TBD if doubly nested
                   Num_Params => Num_Outputs + Block_Inputs'Length,
            ...
            New_Visitor.Last_Awaited_Block := New_Block_Index;
      ...
      if Debug_Code_Gen then
         Put_Line
           (" Creating block" &
            ...
            Block_Index'Image (New_Visitor.Current_Block) &
            " at PC" &
            Code_Offset'Image (New_Visitor.Num_Instrs));
      end if;

      if Instr_Opcode /= Skip_Op then
         --  One or more invokers
         Info.Invokers := new Instr_Loc_Array (0 .. Num_Extra_Invokers);
         Info.Invokers (0).Block := New_Visitor.Current_Block;
         Info.Invokers (0).Instr := New_Visitor.Num_Instrs;
      end if;

      --  Initialize the Block_Visitor
      Block_Visitor :=
        (Trees.Visitor.RW_Tree_Visitor with
         Decl_Region => New_Visitor.Decl_Region,
         Num_Instrs => 0,
         Last_Instr_Escapes => False,
         Op_Sem => New_Visitor.Op_Sem,
         New_Routine => New_Visitor.New_Routine,
         ...
         Nested_Blocks => New_Visitor.Nested_Blocks,
         Current_Code => New_Code,
         Current_Block => New_Block_Index,
         Current_Level => New_Visitor.Current_Level + 1);
      ...
      Block_Visitor.Target_Object :=
        Adjust_For_Level_With_Optional_Temp
          (Current_Visitor => Block_Visitor'Access,
           Obj_Location => New_Visitor.Target_Object,
           Obj_Level => New_Visitor.Current_Level,
           Src_Pos => Find_Source_Pos (Comp_Sem.Definition));
   ...
   procedure Emit_Nested_Block
     (New_Visitor : in out Code_Gen_Visitor;
      ...
      Block_Code : aliased Code_Type (Max_Length);
   begin
      Emit_Nested_Block_Start
        (New_Visitor,
         Block_Visitor,
         Comp_Sem,
         Block_Code'Unchecked_Access,
   ...
   begin
      if New_Visitor.Target_Object = Null_Object_Locator then
      ...
      elsif New_Visitor.Target_Object =
            (Local_Area, New_Visitor.Target_Local_Offset)
      then
      ...
      Emit_Nested_Block_Start
        (New_Visitor,
         ...
         Block_Code'Unchecked_Access,
         Instr_Opcode => Start_Parallel_Op,
         Num_Outputs => Lists.Length (Target_Operation.Operation_Outputs));
      ...
      New_Visitor.Target_Object := Orig_Target_Object;
      ...
      Emit
        (Visitor,
         (Wait_For_Parallel_Op,
          Source_Pos => Source_Pos,
          Parallel_Master => (Local_Area, Visitor.Local_Master),
          Skip_Counts => null));
            ...
            if Is_Implicit_Module_Param then
               ...
               if Visitor.Is_Lvalue_Context then
                  ...
                  if Visitor.Target_Local_Offset >
                    ...
                    or else Visitor.Lvalue_Location /=
                      (Phys_Base_Register (Visitor.Target_Local_Offset - 1), 0)
                  then
         ...
         Emit
           (New_Visitor,
            (Start_Parallel_Call_Op,
             ...
             Num_Params => Num_Outputs + Num_Inputs,
   ...
   procedure Emit_Call
     (Visitor : in out Code_Gen_Visitor;
      ...
      begin
         ...
         elsif Call_Target_Offset /= Final_Target_Offset then
   ...
   begin  --  Emit_Class_Agg
      ...
      if not New_Visitor.Gen_Parallel_Invocations_Only then
         ...
         begin
            ...
            if New_Obj_Offset /= Visitor.Target_Local_Offset then
               ...
               Emit
                 (New_Visitor,
                  (Copy_Word_Op,
                   ...
                   Source => (Local_Area, New_Obj_Offset),
                   Might_Be_Null => False));
            end if;
            ...
            Emit
              (Visitor,
               (Call_Op,
                ...
                Params => (Local_Area, Params_Offset + 2),
                ...
                Static_Link => Run_Time_Type_Info
                                 (Non_Univ_Type,
                                  Referring_Module => Enc_Module),
                Precond_Proved => False,
                Output_Inited_Null => not Key_Type.Known_To_Be_Small));
         end;
      end if;
      ...
      Emit
        (Visitor,
         (Call_Op,
          ...
          Params => (Local_Area, Params_Offset),
          Locked_Param_Info => Locked_Param_Info_For_Container
            (Container_Type => Agg_Sem.Target_Type,
             Container_Param_Index => 2,
             Container_Is_Var => True),
      ...
      if Var_Indexing_Returns_Ref_Obj then
         ...
         Emit
           (Visitor,
            (Copy_Word_Op,
             Source_Pos => Find_Source_Pos (Key),
             Destination => (Local_Area, Ref_Obj_Offset),
             Dest_Name => Strings.Null_U_String_Index,
             Source => (Local_Area, Params_Offset),
             Might_Be_Null => False));
              ...
              (Visitor,
               (Call_Op,
                ...
                Call_Target => Routine_Locator
                                 (Static.Find_One_In_One_Out_Op_For
                                    (Var_Indexing_Output_Type,
                                     Op_Name => Static.Ref_Op_Str),
                                  Var_Indexing_Output_Type),
                Target_Index =>
                  Find_Operation_Routine_Index (Ref_Op_Sem),
                Params => (Local_Area, Params_Offset - 1),
               ...
               Emit
                 (Visitor,
                  (Call_Op,
                   ...
                   Params => (Local_Area, Ref_Obj_Offset),
                   Locked_Param_Info => Null_Locked_Param_Info, --  TBD
                   Static_Link => Run_Time_Type_Info
                                    (Var_Indexing_Output_Type,
                                     Referring_Module => Enc_Module),
   ...
   begin  --  Emit_Container_Agg
      ...
      if Visitor.Target_Object /= Null_Object_Locator then
      ...
      Emit
        (Visitor,
         (Call_Op,
          ...
          Params => (Local_Area, New_Obj_Offset),
          Static_Link => Run_Time_Type_Info
                           (Agg_Sem.Resolved_Type,
                            Referring_Module => Enc_Module),
          Precond_Proved => False,
         ...
         begin
            ...
            if Operand_Tree in For_Loop_Construct.Tree then
               ...
               declare
                  ...
                  Loop_Body_Tree : Trees.Tree'Class renames
                    Tree_Ptr_Of (Loop_Tree.Loop_Body).all;
                  Loop_Iterator : constant Optional_Tree :=
                    Lists.Nth_Element (Loop_Tree.Iterators, 1);
                  Iterator_Tree : Iterator.Tree renames
                    Iterator.Tree (Tree_Ptr_Of (Loop_Iterator).all);
               begin
                           ...
                           (Call_Op,
                            ...
                            Call_Target => Routine_Locator
                                             (Case_Sem.Op_Sem,
                                              Case_Sem.Case_Selector_Type),
                            Target_Index =>
                              Find_Operation_Routine_Index (Case_Sem.Op_Sem),
                        ...
                        Emit
                          (Visitor,
                           (If_Op,
                            ...
                            If_Condition =>
                              Low_Bound_Condition (Key_Binary_Tree.Operator),
                            Skip_If_False => 0));  --  will be filled in later
                        ...
                        Emit
                          (Visitor,
                           (If_Op,
                            ...
                            If_Source => (Local_Area, Param_Offset),
                            If_Condition =>
                              High_Bound_Condition (Key_Binary_Tree.Operator),
                            Skip_If_False => 0));  --  will be filled in later
         ...
         begin
            if Obj_Tree in Identifier.Tree then
               ...
               declare
                  ...
                  Obj_Tree : Trees.Tree'Class renames
               ...
               begin
                  ...
                  then
                     ...
                     return;
                  elsif Obj_Tree in Param_Decl.Tree
                     ...
                     Obj := Resolved_Tree (Case_Construct.Tree (Tree_Ptr_Of
                              (Id_Sem.Associated_Symbol.Enclosing_Region.
                                Associated_Symbol.Definition).all).
                                  Case_Selector);
                  elsif Obj_Tree in Obj_Decl.Tree then
                     --  Is a local declared ref; follow the chain
                     Obj := Resolved_Tree (Obj_Decl.Tree (Obj_Tree).Obj_Value);
                  elsif Obj_Tree in Iterator.Tree then
                     --  Is a reference to a loop parameter, see what
                     --  it refers to.
                     Obj := Resolved_Tree (Iterator.Tree (Obj_Tree).Obj_Value);
                  else
   ...
   procedure Generate_Next_Value
     (Loop_Visitor : in out Code_Gen_Visitor;
      ...
      Continue_With_Value : Optional_Tree := Null_Optional_Tree) is
                         ...
                         Local_Addr =>
                            Adjust_For_Level_And_Prefix
                              (Loop_Visitor.Current_Level,
                               (Local_Area, 0),
                               Iter_Sem.Enclosing_For_Loop.For_Loop_Level)));
                     --  Create null for region associated with where
                     --  master is declared.
                     Emit
                       (Loop_Visitor,
                        (Store_Null_Of_Same_Stg_Rgn_Op,
               ...
               Emit
                 (Loop_Visitor,
                  (Create_Tcb_Op,
                   ...
                   Parallel_Control => (Local_Area, Tcb_Offset),
                   Parallel_Static_Link => Adjust_For_Level_And_Prefix
                                             (Loop_Visitor.Current_Level,
                                              (Local_Area, 0),
                                              For_Loop_Sem.For_Loop_Level),
                   Num_Params => For_Loop_Sem.Iterator_Sems'Length));
            end if;
      ...
      for I in 1 .. Num_Annotations loop
         ...
         begin
            if Annotation_Tree in Trees.Annotation.Tree then
               ...
               --  Not a nested annotation list, so presume it is
   ...
   procedure Emit_Block_For_Check
     (Visitor : in out Code_Gen_Visitor;
      ...
      Max_Length : constant Code_Length_Type := Max_Block_Length;
      Check_Code : aliased Code_Type (Max_Length);
   ...
   begin  --  Emit_Block_For_Check
      ...
      Emit_Nested_Block_Start
        (Visitor,
         Check_Visitor,
         Comp_Sem,
         Check_Code'Unchecked_Access,
         Instr_Opcode => Check_Nested_Block_Op,
         Num_Outputs => 1,
         Assertion_Str =>
            Strings.String_Lookup
              (Subtree_Image (Ann_Tree)));
         ...
         when Null_Literal =>
            ...
            declare
               ...
               Enc_Module : constant Module_Sem_Ptr :=
                 Static.Find_Enclosing_Module_Interface (Visitor.Decl_Region);
            begin
               if Type_Of_Null = Optional_Type then
                 ...
                 and then Visitor.Target_Object /= Null_Object_Locator
               then
   ...
   begin
      ...
      if Master_Offset /= 0 then
         ...
         return Adjust_For_Level_And_Prefix
           (Visitor.Current_Level,
            Obj_Location => (Local_Area, Master_Offset),
               ...
               Emit
                 (Visitor,
                  (Call_Op,
                   ...
                   Params => (Local_Area, Orig_Local_Offset),
                   Locked_Param_Info => Null_Locked_Param_Info,
                   Static_Link => Run_Time_Type_Info
                                    (Comp_Sem.Resolved_Type,
                                     Referring_Module => Enc_Module),
               ...
               case Id_Sym.Kind is
                  when Object_Sym_Kind         |
                     ...
                     begin
                        ...
                        if Obj_Location = Null_Object_Locator then
                           Sem_Error
                             (Id_Sem.Definition,
                        ...
                        if Visitor.Is_Lvalue_Context then
                           ...
                           Emit_Copy_Obj_Or_Word
                             (Visitor,
                              ...
                              Source => Obj_Location,
                              Target_Object => Visitor.Target_Object,
                              Opnd_Sem => Operand_Sem_Ptr (Obj_Sem),
                              Source_Pos =>
                                Find_Source_Pos (Id_Sem.Definition));
                        end if;
                     end;
         ...
         begin
            ...
            then
               ...
               begin
                  ...
                  Cg_Visitor.Current_Code := Routine_Code'Unchecked_Access;
                  ...
                  then
                     ...
                     if Checking_For_Nulls then
                        Check_For_Null_Return (Cg_Visitor,
                          Op_Sem,
                          Source_Pos =>
                  ...
                  if Not_Null (T.Dequeue_Condition) then
                     ...
                     declare
                        ...
                        Dq_Max_Length : constant Code_Length_Type :=
                          Max_Block_Length;
                        Dq_Cond_Code : aliased Code_Type (Dq_Max_Length);
      ...
      if T.Sem_Info /= null then
         ...
         begin
            ...
            if Static.Known_To_Be_Small (Obj_Type) then
               ...
               --  Protect new object
               Visitor.Target_Local_Offset := Visitor.Target_Local_Offset +
                                              1;
            ...
            if Not_Null (T.Obj_Value) then
               ...
               if Static.Sym_Is_By_Ref (New_Obj) or else T.Is_Move then
                  ...
                  if T.Is_Move then
                     ...
                     if Visitor.Lvalue_Location /=
                       (Phys_Base_Register
                         (Obj_Sem.Info.Obj_Location.Offset), 0)
                     then
   ...
   begin
      ...
      case T.Operator is
                 ...
                 (Visitor,
                  (Copy_Word_Op,
                   ...
                   Source =>
                     Adjust_For_Level_And_Prefix
                       (Visitor.Current_Level,
                        Obj_Location => (Local_Area,
                          Visitor.Enclosing_For_Loop.Loop_Result_Local_Offset),
                        Obj_Level =>
         ...
         when Updated_Value_Op =>
            Sem_Error
              (T, "NYI: Updated-value (') operator not yet implemented");
      end case;
   ...
   begin
      if T.Operator = Parallel_Stmt_Op then
         declare
            ...
            Orig_Decl_Region : constant Symbols.Region_Ptr :=
              Visitor.Decl_Region;
         begin
            if Visitor.Gen_Parallel_Invocations_Only then
            ...
            Emit_Nested_Block (Visitor, Comp_Sem, T.Right_Operand);
            ...
            if Binary.Is_Parallel_Stmt_Op (T.Left_Operand) then
               ...
               Emit_Nested_Block (Visitor, Comp_Sem, T.Left_Operand);
            end if;

            --  Restore region
            Visitor.Decl_Region := Orig_Decl_Region;
      ...
      elsif Comp_Sem.all in Call_Semantic_Info then
         --  Treat like a call
         declare
            Call_Sem : constant Call_Sem_Ptr := Call_Sem_Ptr (Comp_Sem);
         begin
            Emit_Call
              (Visitor,
               Call_Sem,
   ...
   begin
      if Visitor.Current_Code /= null
        and then Is_Null (Annotation_Sem.Decl_For_Annotations)
        and then not Lists.Is_Empty (T.Annotations)
      then
         --  Generate "Check_Nested_Block_Op" call as this seems to be
         --  a "simple" statement-level assertion.
         --  TBD: Should have a better way of determining this.

         --  Compute the "slow-calls" info now to determine whether
         --  a master is needed inside the nested block.
         Annotation_Sem.Slow_Calls := Combine_Operand_Calls (T.Annotations);
   ...
   procedure Qualified_Name_Action
     (Visitor : in out Code_Gen_Visitor;
      ...
      Enc_Module : constant Module_Sem_Ptr :=
        Static.Find_Enclosing_Module_Interface (Visitor.Decl_Region);
   begin
      --  TBD: This duplicates almost all of Identifier_Action
      if Visitor.Gen_Parallel_Invocations_Only then
         --  Nothing to do
         if Debug_Code_Gen then
...
--       if U_Sem.all in Literal_Semantic_Info then
--          --  We have a literal of some sort.
--          declare
--             Id : String renames Sym_Name
--                           (Identifier.Tree (Tree_Ptr_Of (T.Id).all));
--          begin
--             Emit_Store_Literal
--               (Visitor,
--                Literal_Sem_Ptr (U_Sem),
--                Id,
--                Source_Pos => Qualified_Name.Find_Source_Pos (T));
--          end;
--       elsif U_Sem.all in Object_Semantic_Info'Class then
--          --  Object or parameter
--          --  TBD: This should be handled more like a "selection" tree
--          --      with the type descriptor acting like the prefix
--          --      of the selection.
--          declare
--             Obj_Sem : constant Object_Sem_Ptr := Object_Sem_Ptr (U_Sem);
--             Is_By_Ref : constant Boolean :=
--               Static.Sym_Is_By_Ref (U_Sem.Associated_Symbol);
--             Obj_Location : Object_Locator :=
--               Adjust_For_Level_With_Optional_Temp
--                  (Visitor'Access,
--                   Obj_Sem.Info.Obj_Location,
--                   Obj_Sem.Info.Obj_Level,
--                   Obj_Ref => Qn_Sem,
--                   Src_Pos => Qualified_Name.Find_Source_Pos (T));
--          begin
--             if Is_By_Ref then
--                --  Is a "ref" object,
--                --  copy address into target-local-offset.
...
--                       Source => Obj_Location));
--
--                   --  Indicate object is through a level of indirection
--                   Obj_Location :=
--                     (Phys_Base_Register (Visitor.Target_Local_Offset), 0);
--
--                   --  Restore target local offset
--                   Check_And_Set_Local_Offset (Visitor, Orig_Offset);
--                end;
--             end if;
--
--             if Visitor.Is_Lvalue_Context then
...
--                   Sem_Error
--                     (T,
--                   "Internal: Object not yet assigned a location for ref");
--                end if;
--
--                Visitor.Lvalue_Location := Obj_Location;
--             elsif Obj_Location = Null_Object_Locator then
--              Sem_Error (T, "Internal: Object not yet assigned a location");
--             else
--                --  Caller wants "content" in Target_Local_Offset
--                Emit_Copy_Obj_Or_Word
--                  (Visitor,
...
--
         ...
         when Operation_Call =>
            ...
            begin
               ...
               if Call_Sem.Resolved_Type /= null
                 and then
                   (Call_Sem.Resolved_Type.Known_To_Be_Small
                      or else
   ...
   begin
      ...
      for I in 1 .. Num_Alts loop
         ...
         begin
            ...
            Visitor.Target_Object := Orig_Target_Object;  -- Set up orig target
   ...
   begin
      ...
      Visitor.Target_Object := Null_Object_Locator;
      Visitor.Dest_Name := Strings.Null_U_String_Index;
      if Orig_Target /= Null_Object_Locator then
      ...
      Emit
        (Visitor,
         (If_Op,
          ...
          If_Source => (Local_Area, Cond_Offset),
          If_Condition => Boolean_Is_True,  --  if True
          Skip_If_False => 0));  --  will be filled in below
   ...
   procedure For_Loop_Construct_Action
     (Visitor : in out Code_Gen_Visitor;
      ...
      Max_Length : constant Code_Length_Type := Max_Block_Length;
      Loop_Code : aliased Code_Type (Max_Length);
      ...
      Loop_Result_Local_Offset : Offset_Within_Area :=
   ...
   begin
      ...
      case T.Kind is
         ...
         when Map_Reduce_Expr =>
            ...
            begin
               ...
               For_Loop_Sem.Loop_Result_Local_Offset :=
                 Loop_Result_Local_Offset;
            ...
            Emit
              (Visitor,
               (Copy_Word_Op,
                ...
                Destination => (Local_Area, Visitor.Target_Local_Offset),
                Dest_Name => Strings.Index
                  (For_Loop_Sem.Iterator_Sems (I).Associated_Symbol.Str),
      ...
      for I in For_Loop_Sem.Iterator_Sems'Range loop
         declare
            ...
            Iter : Iterator.Tree renames Iterator.Tree (Tree_Ptr_Of
                                                           (Iter_Sem.
              Definition).all);
                     ...
                     Emit
                       (Loop_Visitor,
                        (Call_Op,
                         ...
                         Params => Iter_Sem.Info.Obj_Location,
                         Locked_Param_Info => Locked_Param_Info_For_Container
      ...
      case T.Kind is
         ...
         when Map_Reduce_Expr =>
            --  Remember the high-water mark and point to map-reduce result
            Check_And_Set_Local_Offset
              (Visitor, Loop_Result_Local_Offset + 1);

         when Existential_Quantified_Expr | Univ_Quantified_Expr =>
            --  Remember the high-water mark and point to quant-expr result
            Check_And_Set_Local_Offset
              (Visitor, Loop_Result_Local_Offset + 1);
               ...
               Emit
                 (Visitor,
                  (Store_Address_Op,
                   ...
                   Dest_Name => Strings.Index (Iterator_Sym.Str),
                   Source => Visitor.Lvalue_Location,
                   Might_Be_Null =>
                     Resolved_Type (T.Obj_Value).Value_Is_Optional));
            else
               ...
               then
                  ...
                  Emit
                    (Visitor,
                     (Store_Local_Null_Op,
                      Source_Pos => Iterator.Find_Source_Pos (T),
                      Destination => Iterator_Sem.Index_Set_Location,
                      Dest_Name => Strings.Null_U_String_Index,
                      Null_Type_Info => Run_Time_Type_Info
                                          (Iterator_Sem.Index_Set_Type,
                                           Referring_Module => Enc_Module)));
                  ...
                  Emit
                    (Visitor,  --  call "index_set"
                     (Call_Op,
                      ...
                      Params => Iterator_Sem.Index_Set_Location,
                      Locked_Param_Info => Locked_Param_Info_For_Container
                      ...
                      Precond_Proved => False,
                      Output_Inited_Null =>
                        not Iterator_Sem.Index_Set_Type.Known_To_Be_Small));
               end;
      ...
      then
         declare
            ...
            Comp_Location : Object_Locator :=
              (Base_Register (Result_Offset), Comp_Offset);
            Referring_Module : constant Module_Sem_Ptr :=
              Static.Find_Enclosing_Module_Interface (Visitor.Decl_Region);
         ...
         begin
            ...
            if Prefix_Type.Is_Polymorphic then
               ...
               Relevant_Ancestor_Type := Static.Corresponding_Ancestor_Type
                 (Prefix_Type.U_Base_Type, Comp_Module);
               Emit
                 (Visitor,
                  (Select_Polymorphic_Ancestor_Part_Op,
                   ...
                   Polymorphic_Ancestor_Lvalue => Is_Lvalue_Context
                     and then Static.Type_Is_Wrapper
                       (Relevant_Ancestor_Type)));
            ...
            if Static.Type_Is_Wrapper (Relevant_Ancestor_Type) then
               ...
               if Is_Lvalue_Context then
                  ...
                  Comp_Location := (Local_Area, Result_Offset);
               end if;
            end if;
               ...
               Emit
                 (Visitor,
                  (Copy_Word_Op,
                   ...
                   Might_Be_Null => Sel_Sem.Resolved_Type.Value_Is_Optional));
            ...
            elsif Comp_Location /= (Local_Area, Result_Offset)
              or else Target /= Null_Object_Locator
            then
   ...
   procedure Pre_Cg
     (Decl : Optional_Tree;
      ...
      How_Combined : Object_Access.Read_Write_Combination_Enum :=
        Object_Access.Sequential;
      Mode : Object_Access.Access_Mode_Enum := Object_Access.Read_Access) is

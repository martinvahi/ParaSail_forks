...
package body PSC.Trees.Semantics.Info is
   ...
   Debug_Operation_Ids : Boolean := False;
   ...
   begin
      ...
      then
         return Type1.External_View.U_Base_Type =
   ...
   begin
      if Obj_Type = null then
         ...
         return Type_Region_Ptr (Obj_Type.U_Base_Type);
      end if;
   ...
   begin
      while Sem /= null loop
         ...
         begin
            ...
            elsif Sem.all in Annotation_Semantic_Info'Class then
               --  Get sem info from underlying node
               U_Sem :=
                 Annotation_Semantic_Info (Sem.all).Underlying_Sem_Info;
            else
   ...
   begin
      ...
      then
         --  Go to base type, but indicate polymorphic/concurrent
         if Expr_Type.Is_Polymorphic then
            --  Add "+" on end of polymorphic type
            return Canonical_Type_Name (Expr_Type.U_Base_Type) & '+';
         elsif Expr_Type.Obj_Is_Concurrent and then

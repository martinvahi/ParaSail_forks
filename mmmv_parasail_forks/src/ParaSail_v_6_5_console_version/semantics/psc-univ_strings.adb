...
package body PSC.Univ_Strings is
   ...
   Indicator_Multiplier    : constant Univ_String := 2**(24 + 32);
      --  Amount to multiply indicator value to produce Univ_String value

   Null_Value_Indicator    : constant Univ_String := 2;
      --  NOTE: This needs to be consistent with Large_Null_Chunk
   pragma Assert (-Null_Value_Indicator * Indicator_Multiplier =
      Univ_String (Interpreter.Large_Null_Chunk) * Interpreter.Chunk_Divisor);

   U_String_Indicator      : constant Univ_String := 3;
   Short_String_Indicator  : constant Univ_String := 4;

   U_String_Multiplier     : constant Univ_String := 2**32;
      --  Amount to multiply U_String_Index to produce Univ_String value

   Short_String_Multiplier : constant Univ_String := U_String_Multiplier;
      --  Amount to multiply first char of short string to produce Univ_String
   Char_Multiplier         : constant Univ_String := 2**8;
   ...
   U_String_Modulus        : constant Univ_String := 2**24;
   ...
   begin
      if Str < 0 and then Str mod 2 = 1 then
         ...
         declare
            Indicator : constant Univ_String := -Str / Indicator_Multiplier;
         begin
            case -Str / Indicator_Multiplier is
               ...
               when Short_String_Indicator ..
                 Short_String_Indicator + Univ_String'(Max_Short_String) =>
   ...
   begin
      ...
      return -(Univ_String (Rgn) * 2 + Univ_String'(1) +
        Short_String_Indicator * Indicator_Multiplier);
   ...
   begin
      ...
      begin
         return -(Univ_String (Rgn) * 2 + Univ_String'(1) +
           U_String_Indicator * Indicator_Multiplier +
      ...
      if Str < 0 and then Str mod 2 = 1 then
         ...
         begin
            case -Str / Indicator_Multiplier is
               ...
               when Short_String_Indicator + Univ_String'(1) =>
                  --  Single character string
                  return (1 => Character'Val
                                    ((-Str) / Short_String_Multiplier mod
                                       Char_Multiplier));

               when Short_String_Indicator + Univ_String'(2) ..
                 Short_String_Indicator + Univ_String'(Max_Short_String) =>
                  --  Short multi-character string
                  declare
                     ...
                     Chars : Univ_String :=
                       (-Str) / Short_String_Multiplier mod
                                  (Char_Multiplier ** Max_Short_String);
                  begin
         ...
         declare
            ...
            Base : constant Univ_String :=
              -(Univ_String (Rgn) * 2 + Univ_String'(1) +
                 (Short_String_Indicator + Univ_String (Str_Len)) *
            ...
            case Str_Len is
               ...
               when 1 =>
                  return Base - Character'Pos (Str (Str'First)) *
                                  U_String_Multiplier;
               when others =>
                  --  Sum up the characters and subtract from Base
                  declare
                     Sum : Univ_String := 0;
                  begin
         ...
         begin
            case -Str / Indicator_Multiplier is
               ...
               when Short_String_Indicator + Univ_String'(1) =>
                  --  Single character string
                  if Index = 1 then
                     return Word_Type ((-Str) / Short_String_Multiplier mod
                              Char_Multiplier);
                  else
                     return Null_Value;
                  end if;

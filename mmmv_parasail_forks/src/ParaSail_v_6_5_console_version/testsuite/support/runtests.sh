...
compile_and_install_rts_files() {
   if ! test -d $LIB_DIR; then
      echo "error: directory $LIB_DIR not found"
      exit -1
   fi

   if ! test -d $LIB_DIR/aaa; then
      echo "error: directory $LIB_DIR/aaa not found"
      exit -1
   fi

   # ------ Step 1: Ensure that the runtime files are compiled

   pushd $LIB_DIR/aaa

      COMPILER_MAIN="../../build/bin/parasail_main"
      if ! test -f $COMPILER_MAIN; then
         echo "error: $COMPILER_MAIN not available"
         exit -1
      fi

      $COMPILER_MAIN aaa*.ps? ../reflection.ps? \
                              ../vn_il.psi \
                              ../vn_il_stub.psl \
                              ../llvm_printer.ps? \
                              ../compiler.ps? \
                              -command Compile aaa*.ps?

      ./compile_ll.csh aaa*.ps?.ll
   popd

   # ------ Step 2: Copy runtime files into the testsuite

   if test -d $RTS_DIR; then
      rm $RTS_DIR/*
   else
      mkdir $RTS_DIR
   fi

   cp -pr $LIB_DIR/aaa/* $RTS_DIR
   cp -pr $LIB_DIR/reflection.ps? $RTS_DIR
   cp -pr $LIB_DIR/vn_il.psi $LIB_DIR/vn_il_stub.psl $RTS_DIR
   cp -pr $LIB_DIR/llvm_printer.ps? $RTS_DIR
   cp -pr $LIB_DIR/compiler.ps? $RTS_DIR
}
